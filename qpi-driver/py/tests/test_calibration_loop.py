...
class TestWhatTheCalibrationFound:
    """What the calibration actually wrote, checked before any circuit runs against it."""

    def test_the_calibration_finds_the_simulated_chip(self, calibrated_device):
        ...
        assert written["clock_freqs"]["f01"] == pytest.approx(
            simulator.f01 * GHZ, abs=1e6
        )
    ...
    def test_the_executor_loads_exactly_what_the_tuner_wrote(self, calibrated_device):
        device, simulator, scheduler = calibrated_device
        written = yaml.safe_load(device.read_text())["q0"]
...
class TestCircuitsAgainstTheCalibration:
    """Circuits run against a calibrated chip, and what an uncalibrated one gets wrong."""

    def test_an_x_gate_lands_in_the_excited_state(self, calibrated_device):
    ...
    def test_two_x_gates_return_to_the_ground_state(self, calibrated_device):
    ...
    def test_a_hadamard_is_an_even_superposition(self, calibrated_device):
        counts = run(*calibrated_device, circuit("h q[0];\n"), shots=800)
        assert 0.4 < counts["1"] / sum(counts.values()) < 0.6

    def test_an_uncalibrated_chip_gets_the_answer_wrong(self, scheduler, tmp_path):
...
class TestTwoQubitsThroughTheCoordinator:
    ...
    def test_the_parametric_cz_is_parametrised(self, two_qubit_device, tmp_path):
    ...
    def test_the_graph_completes_on_a_plain_transmon_chip(self, scheduler, tmp_path):
    ...
    def test_the_coupler_crossing_is_found_and_a_parking_current_written(
        self, two_qubit_device, tmp_path
    ):
        ...
        assert params["parking_current"] == pytest.approx(
            params["crossing_current"] * 0.6
        )
        written = float(
    ...
    def test_a_mistuned_parametric_coupler_is_found_and_corrected(
        self, two_qubit_device
    ):
        ...
        assert report.status == "success", report.errors
        written = float(
            yaml.safe_load(device.read_text())["q1_q2"]["clock_freqs"]["cz"]
        )
    ...
    def test_a_cz_entangles_two_qubits(self, two_qubit_device):
        ...
        assert 0.35 < q0_excited < 0.65, (
            f"q0's marginal should be ~0.5, got {q0_excited}"
        )

    def test_a_cz_leaves_basis_states_alone(self, two_qubit_device):
    ...
    def test_a_mistimed_cz_does_not_entangle(self, two_qubit_device):
...
class TestVirtualZ:
    """A virtual Z gate's effect on the frame, and that two of them compose."""

    def test_a_virtual_z_actually_rotates_the_frame(self, calibrated_device):
    ...
    def test_two_virtual_z_gates_compose(self, calibrated_device):
...
class TestTheThreeMeasurementLevels:
    ...
    def test_meas_level_0_returns_a_raw_trace(self, calibrated_device):
    ...
    def test_meas_level_1_returns_integrated_iq(self, calibrated_device):
        """One IQ point per shot, and the excited state lands on the excited blob."""
        excited = run_at(
            *calibrated_device, circuit("x q[0];\n"), meas_level=1, shots=200
        )
    ...
    def test_meas_level_1_averaged_collapses_the_shots(self, calibrated_device):
        result = run_at(
            *calibrated_device,
            circuit("x q[0];\n"),
            meas_level=1,
    ...
    def test_meas_level_2_is_discriminated_on_the_instrument(self, calibrated_device):
    ...
    def test_every_measurement_level_agrees_about_the_same_circuit(
        self, calibrated_device
    ):
        ...
        iq = np.asarray(
            run_at(device, simulator, scheduler, qasm, meas_level=1, shots=200)[
                "memory"
            ]
        )
        memory = np.asarray(
...
class TestTheFluxTunableCoupler:
    ...
    def test_the_coupler_carries_its_parking_current(self, coupler_device):
    ...
    def test_the_parking_current_is_bounded(self, coupler_device):
    ...
    def test_a_cz_over_the_coupler_edge_completes_its_exchange(self, coupler_device):
    ...
    def test_the_coupler_makes_the_bell_state_it_should(self, coupler_device):
    ...
    def test_without_its_phase_corrections_the_bell_state_is_wrong(
        ...
        device, simulator, scheduler = coupler_device
        counts = run(
            without_corrections(device, tmp_path),
            simulator,
            scheduler,
            bell_circuit(),
            400,
    ...
    def test_a_coupler_driven_off_resonance_does_nothing(
        self, coupler_device, tmp_path
    ):
    ...
    def test_a_raw_trace_over_two_qubits_takes_one_run_each(self, calibrated_device):
...
class TestTheWholeDagThroughTheRealStack:
    ...
    def test_the_whole_dag_completes_against_the_simulator(self, fully_calibrated):
    ...
    def test_the_ef_pi_pulse_is_measured_through_its_own_clock(self, fully_calibrated):
    ...
    def test_the_ef_drag_coefficient_is_measured_and_negative(self, fully_calibrated):
    ...
    def test_ramsey_refines_f12_past_what_spectroscopy_can_reach(
        self, fully_calibrated
    ):
    ...
    def test_the_dispersive_ladder_is_evenly_spaced(self, fully_calibrated):
    ...
    def test_the_ef_pi_pulse_is_refined_to_a_small_residual_error(
        self, fully_calibrated
    ):
    ...
    def test_leakage_into_the_third_level_is_measured(self, fully_calibrated):
    ...
    def test_the_three_state_readout_point_is_not_the_two_state_one(
        self, fully_calibrated
    ):
    ...
    def test_the_discriminated_readout_point_is_calibrated_and_sane(
        self, fully_calibrated
    ):
    ...
    def test_each_qubit_is_discriminated_against_its_own_line(self, fully_calibrated):
    ...
    def test_the_two_qubit_gate_is_written_and_playable(self, fully_calibrated):
        ...
        assert duration_ns == pytest.approx(
            CoupledTransmons().cz_duration_ns, rel=0.05
    ...
    def test_the_readout_lands_on_the_resonance_at_the_power_it_chose(
        self, fully_calibrated
    ):
    ...
    def test_the_acquisition_window_opens_when_the_signal_arrives(
        self, fully_calibrated
    ):
        ...
        for qubit in ("q0", "q1"):
            ...
            assert written * 1e9 == pytest.approx(
                simulator.time_of_flight_ns, abs=2.0
            ), (
    ...
    def test_resonator_relaxation_measures_the_linewidth_and_writes_nothing(
        ...
        results = [
            ...
            if r.routine_name == "resonator_relaxation"
        ]
    ...
    def test_the_discriminator_is_measured_rather_than_defaulted(
        self, fully_calibrated
    ):
    ...
    def test_f12_was_measured_rather_than_inherited(self, fully_calibrated):
    ...
    def test_punchout_wrote_a_readout_power_inside_its_sweep(self, fully_calibrated):
    ...
    def test_drag_wrote_the_optimum_its_scheduler_spells(self, fully_calibrated):
    ...
    def test_circuits_run_against_what_the_whole_dag_wrote(self, fully_calibrated):
