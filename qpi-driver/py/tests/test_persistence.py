...
class TestSerialisation:
    """Turning a device's elements and edges into the plain dict a YAML dump can hold."""

    def test_an_element_is_written_with_the_class_the_loader_rebuilds_it_from(self):
    ...
    def test_an_edge_is_written_with_the_two_elements_it_joins(self):
    ...
    def test_an_element_is_still_written_with_its_own_name(self):
    ...
    def test_structural_fields_are_not_written_as_calibration(self):
    ...
    def test_an_unreadable_parameter_is_skipped_not_stringified(self):
    ...
    def test_an_object_valued_parameter_is_skipped(self):
        device = FakeDevice(
            elements={
                "q0": FakeElement("q0", submodules={"rxy": FakeSubmodule(amp180=0.1)})
            }
        )
        device._elements["q0"].submodules["rxy"].parameters["obj"] = FakeParameter(
            object()
        )
    ...
    def test_lists_of_scalars_survive(self):
        device = FakeDevice(
            elements={
                "q0": FakeElement("q0", submodules={"p": FakeSubmodule(ports=["a"])})
            }
        )
...
class TestWriting:
    """Writing a device config to disk: the backup it keeps, and the checks a write cannot skip."""

    def test_a_write_produces_plain_safe_yaml(self, tmp_path):
    ...
    def test_the_previous_file_is_kept(self, tmp_path):
        path = tmp_path / "device.yml"
        path.write_text("q0: {}\n")
        save_device_config(_device(), path)
    ...
    def test_the_backup_can_be_restored(self, tmp_path):
        path = tmp_path / "device.yml"
    ...
    def test_restoring_without_a_backup_is_an_error(self, tmp_path):
    ...
    def test_an_empty_device_is_refused(self, tmp_path):
    ...
    def test_verification_rejects_an_element_with_no_type(self, tmp_path):
    ...
    def test_verification_rejects_an_unusable_class_path(self, tmp_path):
        candidate = tmp_path / "candidate.yml"
    ...
    def test_verification_rejects_a_file_that_reads_back_differently(self, tmp_path):
        candidate = tmp_path / "candidate.yml"
        candidate.write_text("q0: {}\n")
    ...
    def test_verification_rejects_a_file_that_is_not_safe_yaml(self, tmp_path):
    ...
    def test_nan_is_not_treated_as_corruption(self):
    ...
    def test_a_device_with_nan_parameters_still_writes(self, tmp_path):
        device = FakeDevice(
            elements={
