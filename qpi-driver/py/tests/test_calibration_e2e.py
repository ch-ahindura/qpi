...
def test_a_full_calibration_walks_the_graph_and_writes_the_device_back(tmp_path):
...
def test_a_full_calibration_measures_coherence_it_does_not_tune(tmp_path):
    ...
    assert measured["t1"]["t1"] == pytest.approx(tuner.simulator.t1_ns * 1e-9, rel=0.05)
    assert measured["t2_echo"]["t2"] == pytest.approx(
        tuner.simulator.t2_ns * 1e-9, rel=0.1
    )


# --- the drift check, and what it queues ---------------------------------------


def test_a_fidelity_check_benchmarks_without_moving_the_calibration(tmp_path):
    tuner = SimulatedTuner()
    config = write_calibration_config(tmp_path)
    element = tuner.device.get_element("q0")
    before = (
    ...
    after = (read_path(element, "clock_freqs.f01"), read_path(element, "rxy.amp180"))
    assert after == before, "a fidelity check measures; it does not calibrate"


def test_drift_queues_a_follow_up_the_worker_can_actually_run(tmp_path):
    ...
    """
    tuner = SimulatedTuner(gate_error=0.02, device_config_path=tmp_path / "device.yml")
    config = write_calibration_config(tmp_path)
    check = {
        "mode": "fidelity_check",
        "job_id": "drift_check",
...
def test_a_healthy_chip_queues_nothing(tmp_path):
...
def test_a_partial_recalibration_leaves_the_other_qubits_alone(tmp_path):
    ...
    report = run_job(
        {"mode": "partial", "job_id": "job-2", "target_qubits": ["q0"]}, tuner, config
    )["report"]
...
def test_a_partial_recalibration_with_no_targets_is_refused(tmp_path):
...
# --- the write-back is a trust boundary ----------------------------------------


def test_a_calibration_that_fails_outright_leaves_the_device_file_alone(tmp_path):
